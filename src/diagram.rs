...
fn create_legend(tree: &mut lowering::MacroRules) -> Option<impl railroad::Node + use<>> {
    ...
    for (fragment, names) in bag {
        ...
        let explanation = match (&fragment, names.len() > 1) {
            ...
            (parser::Fragment::Pat, false) => "is a pattern (including or-patterns).",
            (parser::Fragment::Pat, true) => "are patterns (including or-patterns).",
            (parser::Fragment::PatParam, false) => "is a pattern (without a top-level `|`).",
            (parser::Fragment::PatParam, true) => "are patterns (without a top-level `|`).",
...
fn fragment_to_class(f: &parser::Fragment) -> &'static str {
    match f {
        ...
        parser::Fragment::PatParam => " fragment_pat_param",
